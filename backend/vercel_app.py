...
from backend.models import EnhancedNN, RNNModel, ResNet, CNN_RNN
...
DATA_PATH = REPO_ROOT / "EEG_data.csv"
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS upload_logs
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, filename TEXT,
                  upload_time TEXT, num_rows INTEGER, summary TEXT)''')
    conn.commit()
    conn.close()
...
def startup_event():
    ...
    try:
        ...
        models['rf'] = joblib.load(MODEL_DIR / "rf_model.joblib")

        models['enhanced_nn'] = EnhancedNN(num_classes=2, input_feature_size=36)
        models['enhanced_nn'].load_state_dict(torch.load(MODEL_DIR / "EnhancedNN.pth", map_location=torch.device('cpu'), weights_only=True))
        models['enhanced_nn'].eval()

        models['rnn'] = RNNModel(input_size=36, hidden_size=128, output_size=2)
        models['rnn'].load_state_dict(torch.load(MODEL_DIR / "RNNModel.pth", map_location=torch.device('cpu'), weights_only=True))
        models['rnn'].eval()

        models['resnet'] = ResNet(num_classes=2)
        models['resnet'].load_state_dict(torch.load(MODEL_DIR / "ResNet.pth", map_location=torch.device('cpu'), weights_only=True))
        models['resnet'].eval()

        models['cnn_rnn'] = CNN_RNN(num_classes=2)
        models['cnn_rnn'].load_state_dict(torch.load(MODEL_DIR / "CNN_RNN.pth", map_location=torch.device('cpu'), weights_only=True))
        models['cnn_rnn'].eval()
...
def health_check():
    return {
        ...
        "dataset_loaded": df_full is not None,
    }
...
async def upload_csv(file: UploadFile = File(...)):
    ...
    results = [run_inference(row) for row in batch_features]
...
def get_valid_sessions():
    ...
    return {str(int(k)): [int(x) for x in v.tolist()] for k, v in grouped.items()}
...
def get_sample_schema():
    if df_full is None:
        raise HTTPException(status_code=500, detail="Dataset not loaded")
    return df_full[FEATURE_COLS].iloc[0].to_dict()
